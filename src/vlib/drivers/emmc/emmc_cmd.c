...
static void emmc_data_transfer_dma(void)
{
	...
	if (mmc_drv_obj.cmd_info.dir == HAL_MEMCARD_WRITE) {
		/* flush cache */
        R_OSAL_CacheFlush((size_t)mmc_drv_obj.buff_address_virtual, mmc_drv_obj.trans_size, (size_t)R_OSAL_CACHE_D);
	...
	/* DMA Transfer Destination/Source Address */
	...
	if (mmc_drv_obj.cmd_info.dir == HAL_MEMCARD_READ) {
		/* Invalidate cache during DMA transfer */
		R_OSAL_CacheInvalidate((size_t)mmc_drv_obj.buff_address_virtual, mmc_drv_obj.trans_size, (size_t)R_OSAL_CACHE_D);
	}
}
...
EMMC_ERROR_CODE emmc_exec_cmd(uint32_t error_mask, uint32_t *response)
{
	...
	while ((mmc_drv_obj.force_terminate != TRUE) && (state != ESTATE_END)) {
		...
		case ESTATE_RCV_RESP:
			...
			/* R-Car3 WA: adjust hs400 offset for R-Car M3N, M3W1.3, E3 */
			if ((mmc_drv_obj.need_adjust_hs400) &&
				(CMD13_SEND_STATUS == mmc_drv_obj.cmd_info.cmd)) {
				if (response[0] & EMMC_R1_READY) {
					emmc_hs400_adjust_enable();
				}
			}
		...
		case ESTATE_ERROR:
			if (err_not_care_flag == TRUE || mmc_drv_obj.tuning_status == EMMC_TUNING_DOING) {
				mmc_drv_obj.during_cmd_processing = FALSE;
			} else {
				emmc_softreset();
				emmc_write_error_info(EMMC_FUNCNO_EXEC_CMD,
