...
/*----------------------------------------------------------------------------------------------------------*/
/* Internal type for integer access to rhw data.*/
/**/
typedef union tagdhd_internal_rhw_data {  
  struct dhd_rhw_float32_t {
    dhd_float32_t                m_start;                                 /* start value*/
    dhd_float32_t                m_dx, m_dy;                              /* increment  */
  } m_float32;                                                            /* PRQA S 3629 *//* $Misra: Intended uint access to float data for performance optimization. $*/
  struct dhd_rhw_uint32_t {
    dhd_uint32_t                 m_start_i;                               /* int 32 aliases for start value*/
    dhd_uint32_t                 m_dx_i, m_dy_i;                          /* int 32 aliases for increment  */
  } m_uint32;
} dhd_internal_rhw_data_t;

/*---------------------------------------------------------------------------------------------------------- */
/* Specify a fixed rhw gradient. Similar to dhd_set_uv when an rhw gradient does not change frequently it can */
/* be set up using this function rather than beeing repeated for each primitive. */
/* */
/* Parameters: */
/*  a_device : Driver context handle (as returned by dhd_init) */
/*  a_unitid : Index of the texture unit to update (0..7) */
/*  a_rhw : Pointer to uv data (see dhd_rhw_data_t) */
DHD_API_INLINE void dhd_set_rhw(dhd_handle_t a_device, dhd_uint32_t a_unitid, const dhd_rhw_data_t *a_rhw) { 
  dhd_device_data_t *device = dhd_get_device( a_device );
  dhd_uint32_t regindex;
  dhd_uint32_t *rhwlim;
  const dhd_internal_rhw_data_t *rhw_i = (dhd_internal_rhw_data_t *) a_rhw;

  /* validate uv unit index */
  if (a_unitid >= device->m_description.m_num_tex_units) {
    return; /* error*/
  }

  // get register index of DHD_REG_PSU_LIM_VSTART_n relative to group4 start
  regindex = (DHD_REG_TXA_RHW_OFFSET_0 + (a_unitid * 3)) - (4 * 64);
  rhwlim = & device->m_shadow_state.m_groups.m_group4[ regindex ];

  rhwlim[ 0 ] = rhw_i->m_uint32.m_start_i;
  rhwlim[ 1 ] = rhw_i->m_uint32.m_dx_i;
  rhwlim[ 2 ] = rhw_i->m_uint32.m_dy_i;

  /* set relevant pending bits (3 consecutive bits)  */
  device->m_pending_mask.m_groups.m_group4[ 0 ] |= 0x7u << regindex; 
}
