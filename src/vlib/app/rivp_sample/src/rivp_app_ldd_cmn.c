...
/*                    FORWARD DECLARATION                                     */
/******************************************************************************/
RIVP_LDD_STATIC void RIVP_AppLddCmnConvLinear(RIVP_APP_LDD_CMN_XY_T *xy);
RIVP_LDD_STATIC void RIVP_AppLddCmnXy2Linear (RIVP_APP_LDD_CMN_XY_T *xy);
RIVP_LDD_STATIC void RIVP_AppLddCmnClipLinear(RIVP_APP_LDD_CMN_XY_T *xy);
...
/**
 * \brief			convert picture data to linear format
 *
 * \param[in,out]	app		  : application control information
 * \param[in]		file	  : input/output information (unused)
 *
 * \retval			none
 *
 * \attention 		only a check using rivp_app_strm_avc_conf025_264_5fr/jpg_cjpeg_000.h.
 ...
 *
 */
void RIVP_AppLddCmnOutputData(
	RIVP_APP_LDD_CMN_PARAM_T *app,
	RIVP_APP_LDD_CMN_FILE_T *file)
{
	/* convert tile to linear */
	RIVP_AppLddCmnConvLinear(&app->xy[RIVP_APP_LDD_CMN_XY_LUMA]);
	RIVP_AppLddCmnConvLinear(&app->xy[RIVP_APP_LDD_CMN_XY_CHROMA]);
	/* output YUV file */
	/* not implement */
}
	...
	RIVP_APP_LDD_CMN_PARAM_T *app)
{
	...
	if (app != RIVP_LDD_NULL) {
		...
		if (lBoolRet == RIVP_LDD_TRUE) {
			...
			app->taskHnd[RIVP_APP_LDD_OSAL_DEC_TASK_ID].priority = RIVP_APP_LDD_OSAL_TASK_PRIORITY;
		...
		if (lBoolRet == RIVP_LDD_TRUE) {
			...
			app->taskHnd[RIVP_APP_LDD_OSAL_OUT_TASK_ID].priority = RIVP_APP_LDD_OSAL_TASK_PRIORITY;
...
/**
 * \brief			convert tile/linear to clipped linear
 *
 * \param[in,out]	xy		    : convert information structure
 *
 * \retval			none
 *
 * \attention 		only a check using rivp_app_strm_avc_conf025_264_5fr/jpg_cjpeg_000.h.
 ...
 *
 */
RIVP_LDD_STATIC void RIVP_AppLddCmnConvLinear(
	RIVP_APP_LDD_CMN_XY_T *xy)
{
	if (xy != RIVP_LDD_NULL) {
		if (xy->xyLineMode == RIVP_APP_LDD_CMN_LN_OUT) {
			RIVP_AppLddCmnClipLinear(xy);
		} else {
			RIVP_AppLddCmnXy2Linear(xy);
		}
	}
}

/**
 * \brief			convert linear to clipped linear
 *
 * \param[in,out]	xy		    : convert information structure
 *
 * \retval			none
 ...
 *
 */
RIVP_LDD_STATIC void RIVP_AppLddCmnClipLinear(
	RIVP_APP_LDD_CMN_XY_T *xy)
{
	RIVP_LDD_U32  lHCnt;
	RIVP_LDD_U32  lVCnt;
	RIVP_LDD_U32  lXValidSize;
	RIVP_LDD_U32  lYValidSize;
	RIVP_LDD_U8	 *lSrcPtr;
	...
	if (xy != RIVP_LDD_NULL) {
		lXValidSize = xy->xPicSize;
		lYValidSize = xy->yPicSize;

		if (xy->lumaChroma == RIVP_APP_LDD_CMN_XY_LUMA) {
			for (lVCnt = 0u; lVCnt < lYValidSize; lVCnt++) {
				if (xy->xyOnField == 0u) {
					/* Frame */
					lSrcPtr = (RIVP_LDD_U8 *)( (RIVP_LDD_ADR)xy->xyTopAddr + xy->cropLeft + ( (lVCnt + xy->cropTop) * xy->stride ) );
				} else {
					if ((lVCnt & 1u) == 0u) {
						lSrcPtr = (RIVP_LDD_U8 *)( (RIVP_LDD_ADR)xy->xyTopAddr + xy->cropLeft + ( ((lVCnt >> 1u) + xy->cropTop) * xy->stride ) );
					} else {
						lSrcPtr = (RIVP_LDD_U8 *)( (RIVP_LDD_ADR)xy->xyBotAddr + xy->cropLeft + ( ((lVCnt >> 1u) + xy->cropTop) * xy->stride ) );
					}
				}
				lDst = (RIVP_LDD_U8 *)( (RIVP_LDD_ADR)xy->lAddr[0]  + (lVCnt * lXValidSize) );
				(void)memcpy(lDst, lSrcPtr, lXValidSize);
			}
		} else {
			lDst0 = (RIVP_LDD_U8 *)xy->lAddr[0];
			lDst1 = (RIVP_LDD_U8 *)xy->lAddr[1];
			...
			for (lVCnt = 0u; lVCnt < lYValidSize; lVCnt++) {
				...
				if (xy->bytePerPixel == 1u) {
					for (lHCnt = 0u; lHCnt < (lXValidSize/ 2u); lHCnt++) {
						*lDst0++ = *lSrcPtr++;
						*lDst1++ = *lSrcPtr++;
					}
				} else {
					for (lHCnt = 0u; lHCnt < (lXValidSize / 4u); lHCnt++) {
						*lDst0++ = *lSrcPtr++;
						*lDst0++ = *lSrcPtr++;
						*lDst1++ = *lSrcPtr++;
						*lDst1++ = *lSrcPtr++;
...
/**
 * \brief			convert tile to clipped linear
 *
 * \param[in,out]	xy		    : convert information structure
 *
 * \retval			none
 ...
 *
 */
RIVP_LDD_STATIC void RIVP_AppLddCmnXy2Linear(
	RIVP_APP_LDD_CMN_XY_T *xy)
{
	static RIVP_LDD_U32 s_rivpAppLddCmnXyCache[RIVP_APP_LDD_CMN_SWAP_FRAME_SIZE / sizeof(RIVP_LDD_U32)];

	RIVP_LDD_U8		*lDst0;
	RIVP_LDD_U8		*lDst1;
	RIVP_LDD_U32 	*lDstLong;

	RIVP_LDD_U32 	 lXCnt;
	RIVP_LDD_U32 	 lYCnt;
	RIVP_LDD_U32 	 lTileOffsetH;
	RIVP_LDD_U32 	 lTileOffsetV;

	RIVP_LDD_U32 	 l4ByteCopy;
	RIVP_LDD_U32 	 lXLeft;
	RIVP_LDD_U32 	 lXRight;
	RIVP_LDD_U32 	 lYTop;
	RIVP_LDD_U32 	 lYBot;
	RIVP_LDD_U32 	 lActPixelInLine;
	RIVP_LDD_U32 	 lYCopySize;
	RIVP_LDD_U32 	 lLog2tmp;
	RIVP_LDD_U32 	 lLog2stride;

	RIVP_LDD_U8 	*lSrcFrame;
	RIVP_LDD_U8 	*lSrcTop;
	RIVP_LDD_U8 	*lSrcBot;

	RIVP_LDD_BOOL	 lErrFlg;

	if (xy == RIVP_LDD_NULL) {
		lErrFlg = RIVP_LDD_TRUE;
	} else {
		lErrFlg = RIVP_LDD_FALSE;
	}

	if (lErrFlg != RIVP_LDD_TRUE) {
		if ((xy->yPicSize <= (xy->cropTop  + xy->cropBot  )) ||
			(xy->xPicSize <= (xy->cropLeft + xy->cropRight))) {
			lErrFlg = RIVP_LDD_TRUE;;
		}
	}
	...
	if (lErrFlg != RIVP_LDD_TRUE) {
		...
		lYTop			= xy->cropTop;
		lYBot			= ( xy->yPicSize - xy->cropBot );
		lXLeft			= xy->cropLeft;
		lXRight			= ( xy->xPicSize - xy->cropRight );
		lActPixelInLine	= lXRight - lXLeft;
		lYCopySize		= xy->yPicSize;

		if (xy->xyOnField) { /* Field */
			lYBot /= 2u;
			if (xy->_2lineShift) {
				lYTop	+= 2u;
				lYBot	+= 2u;
				lYCopySize = xy->yPicSize + 4u;
			}
		}

		if (((lXLeft  % sizeof(RIVP_LDD_U32)) == 0u) &&
			((lXRight % sizeof(RIVP_LDD_U32)) == 0u)) {
			l4ByteCopy = 1u;
		} else{
			l4ByteCopy = 0u;
		}
	...
	if (lErrFlg != RIVP_LDD_TRUE) {
		if (xy->lumaChroma == RIVP_APP_LDD_CMN_XY_LUMA) {
			/* Y */
			if (xy->xyOnField == 0u) { /* Frame */
				lSrcFrame = (RIVP_LDD_U8 *)s_rivpAppLddCmnXyCache;
				(void)memcpy( lSrcFrame, xy->xyTopAddr, xy->stride * RIVP_APP_LDD_ALIGN( lYCopySize, 32u ) );

				if (l4ByteCopy) {
					lDstLong	= (RIVP_LDD_U32 *)xy->lAddr[0];
					for (lYCnt = lYTop; lYCnt < lYBot; lYCnt++) {
						RIVP_APP_LDD_CMN_TL_CONV_Y( lLog2stride, lYCnt );
						for (lXCnt = lXLeft; lXCnt < lXRight; lXCnt += sizeof(RIVP_LDD_U32)) {
							RIVP_APP_LDD_CMN_TL_CONV_X( lXCnt );
							*lDstLong++ = *(RIVP_LDD_U32 *)( lSrcFrame + lTileOffsetV + lTileOffsetH );
						}
					}
				} else {
					lDst0		= xy->lAddr[0];
					for (lYCnt = lYTop; lYCnt < lYBot; lYCnt++) {
						RIVP_APP_LDD_CMN_TL_CONV_Y( lLog2stride, lYCnt );
						for (lXCnt = lXLeft; lXCnt < lXRight; lXCnt++) {
							RIVP_APP_LDD_CMN_TL_CONV_X( lXCnt );
							*lDst0++ = *( lSrcFrame + lTileOffsetV + lTileOffsetH );
						}
					}
				}
			} else { /* Field */

				lSrcTop = (RIVP_LDD_U8 *)s_rivpAppLddCmnXyCache;
				lSrcBot = lSrcTop + xy->stride * RIVP_APP_LDD_ALIGN( lYCopySize, 64u ) / 2u;

				(void)memcpy( lSrcTop, xy->xyTopAddr, xy->stride * RIVP_APP_LDD_ALIGN( lYCopySize, 64u ) / 2u );
				(void)memcpy( lSrcBot, xy->xyBotAddr, xy->stride * RIVP_APP_LDD_ALIGN( lYCopySize, 64u ) / 2u );
				...
				if (l4ByteCopy) {
					...
					for (lYCnt = lYTop; lYCnt < lYBot; lYCnt++) {
						...
						for (lXCnt = lXLeft; lXCnt < lXRight; lXCnt += sizeof(RIVP_LDD_U32)) {
							RIVP_APP_LDD_CMN_TL_CONV_X( lXCnt );
							*lDstLong++ = *(RIVP_LDD_U32 *)( lSrcTop + lTileOffsetV + lTileOffsetH );
						}
						lDstLong += ( lActPixelInLine / sizeof(RIVP_LDD_U32) );
					}
					/* bot */
					lDstLong	= (RIVP_LDD_U32 *)( xy->lAddr[0] + lActPixelInLine );
					for (lYCnt = lYTop; lYCnt < lYBot; lYCnt++) {
						RIVP_APP_LDD_CMN_TL_CONV_Y( lLog2stride, lYCnt );
						for (lXCnt = lXLeft; lXCnt < lXRight; lXCnt += sizeof(RIVP_LDD_U32)) {
							RIVP_APP_LDD_CMN_TL_CONV_X( lXCnt );
							*lDstLong++ = *(RIVP_LDD_U32 *)( lSrcBot + lTileOffsetV + lTileOffsetH );
						}
						lDstLong += ( lActPixelInLine / sizeof(RIVP_LDD_U32) );
					}
				} else {
					...
					for (lYCnt = lYTop; lYCnt < lYBot; lYCnt++) {
						...
						for (lXCnt = lXLeft; lXCnt < lXRight; lXCnt++) {
							RIVP_APP_LDD_CMN_TL_CONV_X( lXCnt );
							*lDst0++ = *( lSrcTop + lTileOffsetV + lTileOffsetH );
						}
						lDst0 += lActPixelInLine;
					}
					/* bot */
					lDst0	= ( xy->lAddr[0] + lActPixelInLine );
					for (lYCnt = lYTop; lYCnt < lYBot; lYCnt++) {
						RIVP_APP_LDD_CMN_TL_CONV_Y( lLog2stride, lYCnt );
						for (lXCnt = lXLeft; lXCnt < lXRight; lXCnt++) {
							RIVP_APP_LDD_CMN_TL_CONV_X( lXCnt );
							*lDst0++ = *( lSrcBot + lTileOffsetV + lTileOffsetH );
						}
						lDst0 += lActPixelInLine;
					}
				}
			...
			if (xy->xyOnField == 0u) { /* Frame */
				...
				for (lYCnt = lYTop; lYCnt < lYBot; lYCnt++) {
					RIVP_APP_LDD_CMN_TL_CONV_Y( lLog2stride, lYCnt );
					if (xy->bytePerPixel == 1u) {
						for (lXCnt = lXLeft; lXCnt < lXRight; lXCnt += 2u) {
							RIVP_APP_LDD_CMN_TL_CONV_X( lXCnt );
							*lDst0++				= *( lSrcFrame + lTileOffsetV + lTileOffsetH );
							*lDst1++				= *( lSrcFrame + lTileOffsetV + lTileOffsetH + 1u );
						}
					} else {
						for (lXCnt = lXLeft; lXCnt < lXRight; lXCnt += 4u) {
							RIVP_APP_LDD_CMN_TL_CONV_X( lXCnt );
							*lDst0++				= *( lSrcFrame + lTileOffsetV + lTileOffsetH      );
							*lDst0++				= *( lSrcFrame + lTileOffsetV + lTileOffsetH + 1u );
							*lDst1++				= *( lSrcFrame + lTileOffsetV + lTileOffsetH + 2u );
							*lDst1++				= *( lSrcFrame + lTileOffsetV + lTileOffsetH + 3u );
						}
					}
				}
			} else { /* Field */
				...
				for (lYCnt = lYTop; lYCnt < lYBot; lYCnt++) {
					...
					if (xy->bytePerPixel == 1u) {
						for (lXCnt = lXLeft; lXCnt < lXRight; lXCnt += 2u) {
							RIVP_APP_LDD_CMN_TL_CONV_X( lXCnt );
							*lDst0++				= *( lSrcTop + lTileOffsetV + lTileOffsetH );
							*lDst1++				= *( lSrcTop + lTileOffsetV + lTileOffsetH + 1u );
						}
					} else {
						for (lXCnt = lXLeft; lXCnt < lXRight; lXCnt += 4u) {
							RIVP_APP_LDD_CMN_TL_CONV_X( lXCnt );
							*lDst0++				= *( lSrcTop + lTileOffsetV + lTileOffsetH      );
							*lDst0++				= *( lSrcTop + lTileOffsetV + lTileOffsetH + 1u );
							*lDst1++				= *( lSrcTop + lTileOffsetV + lTileOffsetH + 2u );
							*lDst1++				= *( lSrcTop + lTileOffsetV + lTileOffsetH + 3u );
						}
					}
					lDst0 += lActPixelInLine / 2u;
					lDst1 += lActPixelInLine / 2u;
				}

				/* bot */
				lDst0		= (xy->lAddr[0] + lActPixelInLine / 2u);
				lDst1		= (xy->lAddr[1] + lActPixelInLine / 2u);
				for (lYCnt = lYTop; lYCnt < lYBot; lYCnt++) {
					RIVP_APP_LDD_CMN_TL_CONV_Y( lLog2stride, lYCnt );
					if (xy->bytePerPixel == 1u) {
						for (lXCnt = lXLeft; lXCnt < lXRight; lXCnt += 2u) {
							RIVP_APP_LDD_CMN_TL_CONV_X( lXCnt );
							*lDst0++				= *( lSrcBot + lTileOffsetV + lTileOffsetH );
							*lDst1++				= *( lSrcBot + lTileOffsetV + lTileOffsetH + 1u );
						}
					} else {
						for (lXCnt = lXLeft; lXCnt < lXRight; lXCnt += 4u) {
							RIVP_APP_LDD_CMN_TL_CONV_X( lXCnt );
							*lDst0++				= *( lSrcBot + lTileOffsetV + lTileOffsetH      );
							*lDst0++				= *( lSrcBot + lTileOffsetV + lTileOffsetH + 1u );
							*lDst1++				= *( lSrcBot + lTileOffsetV + lTileOffsetH + 2u );
							*lDst1++				= *( lSrcBot + lTileOffsetV + lTileOffsetH + 3u );
						}
					}
					lDst0 += lActPixelInLine / 2u;
					lDst1 += lActPixelInLine / 2u;
