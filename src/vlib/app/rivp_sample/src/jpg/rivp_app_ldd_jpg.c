 ...
 *
 * PURPOSE
 *   RIVP Reference Application JPG Decoder
 *
 * AUTHOR
...
/*                    FORWARD DECLARATION                                     */
/******************************************************************************/
RIVP_LDD_STATIC RIVP_LDD_BOOL	 RIVP_AppLddJpgInitAppInfo(RIVP_APP_LDD_CMN_PARAM_T **app, RIVP_LDD_BOOL existResource);
RIVP_LDD_STATIC RIVP_LDD_RESULT	 RIVP_AppLddJpgInitLibProc(RIVP_APP_LDD_CMN_PARAM_T *app);

/******************************************************************************/
/*                         VARIABLE                                           */
/******************************************************************************/
RIVP_LDD_STATIC RIVP_LDD_U8		 s_rivpAppLddJpgYuvBuff[RIVP_APP_LDD_YBUF_SIZE + (RIVP_APP_LDD_CBUF_SIZE * RIVP_APP_LDD_CMN_XY_MAX)] = {0u};
 ...
 *
 */
RIVP_LDD_RESULT RIVP_AppLddJpgMain( void )
{
	static RIVP_APP_LDD_CMN_PARAM_T	*lAppPtr	  = RIVP_LDD_NULL;
	static RIVP_LDD_BOOL			 lResourceFlg = RIVP_LDD_FALSE;
	const RIVP_LDD_VERSION_T		*lVerPtr;
	RIVP_LDD_RESULT					 lResult;
	RIVP_LDD_BOOL					 lBoolRet;
	const RIVP_LDD_JPG_VERSION_T	*lJpgVer;
	RIVP_LDD_U32					 lCnt;
	...
	RIVP_LDD_LOG_APP_INF("!!! RIVP Reference Application JPG Decoder start !!!\n");

	/* initialize application control information */
	lBoolRet = RIVP_AppLddJpgInitAppInfo(&lAppPtr, lResourceFlg);

	/* initialize this library */
	if (lBoolRet == RIVP_LDD_TRUE) {
		lResult = RIVP_AppLddJpgInitLibProc(lAppPtr);
	} else {
	...
	if (lResult == RIVP_LDD_RTN_OK) {
		...
		/* get version */
		RIVP_AppLddCmnGetCurTime(&lAppPtr->perf.funcStart);
		lJpgVer = RIVP_LddJpgGetVersion();
		RIVP_AppLddCmnGetProcTime(lAppPtr, RIVP_APP_LDD_FUNC_JPG_GETVER);
		
		RIVP_LDD_LOG_APP_INF("RIVP_LddJpgGetVersion() = %s\n",	  g_rivpAppLddCmnResCode[(RIVP_LDD_U32)RIVP_LDD_RTN_OK]);
		RIVP_LDD_LOG_APP_INF(" - major    = %2d\n", lJpgVer->major);
		RIVP_LDD_LOG_APP_INF(" - minor    = %2d\n", lJpgVer->minor);
		RIVP_LDD_LOG_APP_INF(" - patch    = %2d\n", lJpgVer->patch);
	}
	...
	if (lResult == RIVP_LDD_RTN_OK) {
		if (lResourceFlg != RIVP_LDD_TRUE) {
			/* create OS resources and run tasks */
			lBoolRet = RIVP_AppLddCmnCreateResource(lAppPtr);

			/* update flag to avoid duplication */
			lResourceFlg = RIVP_LDD_TRUE;
		} else {
			/* restart tasks */
			lBoolRet = RIVP_AppLddCmnRestartTask(lAppPtr);
		}

		/* wait until receiving message */
		if (lBoolRet == RIVP_LDD_TRUE) {
			lBoolRet = RIVP_AppLddOsalQueueReceive(&lAppPtr->msgInf[RIVP_APP_LDD_MAIN_MSG_QUEUE], &lMsg[0]);
	...
	if (lBoolRet == RIVP_LDD_TRUE) {
		/* compare YUV data */
		for (lCnt = 0u; ((lCnt < sizeof(g_rivpAppLddJpgSampleYuv)) && (lBoolRet != RIVP_LDD_FALSE)); lCnt++) {
			if (lAppPtr->yuvBuff[lCnt] != g_rivpAppLddJpgSampleYuv[lCnt]) {
	...
	if (lBoolRet == RIVP_LDD_TRUE) {
		if (lLoopFlg != RIVP_LDD_TRUE) {
			RIVP_AppLddCmnMemFree(lAppPtr, lAppPtr->file, lAppPtr->codecConfig);
		}
	}

	/* output result log */
	RIVP_LDD_LOG_APP_INF("***************************************************\n");
	RIVP_LDD_LOG_APP_INF("    RIVP_AppLddJpgMain result=%s \n", g_rivpAppLddCmnResCode[(RIVP_LDD_U32)lResult]);
	RIVP_LDD_LOG_APP_INF("***************************************************\n\n");


	RIVP_LDD_LOG_APP_INF("!!! RIVP Reference Application JPG Decoder end !!!\n\n");
...
RIVP_LDD_STATIC RIVP_LDD_BOOL RIVP_AppLddJpgInitAppInfo(RIVP_APP_LDD_CMN_PARAM_T **app, RIVP_LDD_BOOL existResource)
{
	...
	if (app != RIVP_LDD_NULL) {
		if (existResource == RIVP_LDD_FALSE) {
			if (sizeof(RIVP_LDD_JPG_CONFIG_T) <= RIVP_APP_LDD_CODEC_CONFIG_SIZE) {
				/* allocate application resource */
				lBoolRet = RIVP_AppLddCmnMemAlloc(&lAppPtr, &lFilePtr, (RIVP_LDD_U32**)&lJpgConfigPtr);
			}
			...
			if (lBoolRet == RIVP_LDD_TRUE) {
				...
				*app                 = lAppPtr;
				...
				lAppPtr->codecConfig = lJpgConfigPtr;
		...
		if (lBoolRet == RIVP_LDD_TRUE) {
			...
			lAppPtr->useRingBuff		= RIVP_LDD_FALSE;	/* use ring buffer, or not */
			lAppPtr->picIndex			= 0u;
			lAppPtr->strmInfo.strmId	= 0u;
			lAppPtr->errorCode			= RIVP_LDD_RTN_OK;
			lAppPtr->yuvBuff			= s_rivpAppLddJpgYuvBuff;
			lAppPtr->yuvBuffCnt			= 0u;
			lAppPtr->streamFrameId		= RIVP_APP_LDD_JPG_DATA_STRM_FRAME;

			/* set input stream information */
			lAppPtr->file->fSrc			= g_rivpAppLddJpgSampleStrm;
			lAppPtr->file->fSize		= sizeof(g_rivpAppLddJpgSampleStrm);
		}
	}
...
RIVP_LDD_STATIC RIVP_LDD_RESULT RIVP_AppLddJpgInitLibProc(RIVP_APP_LDD_CMN_PARAM_T *app)
{
	...
	if (app != RIVP_LDD_NULL) {
		...
		if (lResult == RIVP_LDD_RTN_OK) {
			/* get entry point of JPEG. */
			RIVP_AppLddCmnGetCurTime(&app->perf.funcStart);
			lResult = RIVP_LddJpgGetEntryPoint(&app->ctxParam.config.codecEntryPoint);
			RIVP_AppLddCmnGetProcTime(app, RIVP_APP_LDD_FUNC_JPG_ENTRY);
			
			RIVP_LDD_LOG_APP_INF("RIVP_LddJpgGetEntryPoint() = %s\n", g_rivpAppLddCmnResCode[(RIVP_LDD_U32)lResult]);
		}
	}
