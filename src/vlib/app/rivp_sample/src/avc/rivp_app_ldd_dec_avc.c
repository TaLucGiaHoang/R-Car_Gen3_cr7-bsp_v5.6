...
RIVP_LDD_STATIC RIVP_LDD_BOOL RIVP_AppLddDecAvcInitCtxParam(RIVP_APP_LDD_CMN_PARAM_T *app)
{
	...
	if (app != RIVP_LDD_NULL) {
		...
		lAvcConfigPtr->reorderEnable			= RIVP_LDD_FALSE;						/* TRUE,FALSE */ /* [T.B.D.] TRUE case is not tested in this application */
...
RIVP_LDD_STATIC RIVP_LDD_RESULT RIVP_AppLddDecAvcInput(RIVP_APP_LDD_CMN_PARAM_T *app)
{
	RIVP_LDD_RESULT	 lResult;
	RIVP_LDD_BOOL	 lBoolRet;
	RIVP_LDD_BOOL	 lDecodingFlg;
	RIVP_LDD_BOOL	 lEosInput;
	RIVP_LDD_U8		 lMsg[RIVP_APP_LDD_SIZE_MAIN_DEC_MSG];
		...
		while (lDecodingFlg == RIVP_LDD_TRUE) {
			...
			if (app->errorCode != RIVP_LDD_RTN_OK) {
				...
				if (app->firstInput == RIVP_LDD_TRUE) {
					...
					memcpy(&lDecodingFlg, &lMsg[0], sizeof(lDecodingFlg));
				...
				if (lBoolRet == RIVP_LDD_TRUE) {
					...
					lDecodingFlg = RIVP_AppLddDecAvcSearchStartCode(app, app->searchMode);
				} else {
					RIVP_LDD_LOG_APP_ERR_SYS("Failed to RIVP_AppLddDecAvcInput. Receive message error.\n");
					lDecodingFlg = RIVP_LDD_FALSE;
				}

				if (lDecodingFlg == RIVP_LDD_TRUE) {
					/* input */
					...
					if ((((RIVP_LDD_AVC_CONFIG_T*)app->ctxParam.config.codecConfig)->reorderEnable == RIVP_LDD_TRUE) &&
						(lEosInput == RIVP_LDD_FALSE)) {
						/* in the case of reorderEnable==TRUE, it is necessary for application to input EOS without EOS in the stream terminal. */
						app->input.strmBuffNum			 = 1u;
						app->input.strmBuff[0].virtAddr	 = app->strmEos;
						app->input.strmBuff[0].physAddr	 = RIVP_APP_LDD_EOS_ID; /* dummy address. not use RIVP. */
						app->input.strmBuff[0].size		 = (RIVP_LDD_U32)sizeof(app->strmEos);
						app->input.strmId				 = RIVP_APP_LDD_EOS_ID;

						lEosInput    = RIVP_LDD_TRUE;
						lDecodingFlg = RIVP_LDD_TRUE;
