...
#ifndef RIVP_APP_DISPLAY
#define RIVP_APP_DISPLAY        (RIVP_LDD_TRUE)
#endif
...
#if (RIVP_APP_DISPLAY != RIVP_LDD_FALSE)
#if (RIVP_APP_AS_TOTALNUM != 1u)
#error    /* multiple display do not support */
#endif
#endif
...
RIVP_LDD_RESULT RIVP_AppLddAvcMain( void )
{
	...
	if (lBoolRet == RIVP_LDD_TRUE) {
		/* sleep for wait input thread sent to RIVP_APP_MAIN_MSG_QUEUE over */
		RIVP_AppLddOsalUSleep(10000u);
...
RIVP_LDD_STATIC RIVP_LDD_BOOL RIVP_AppLddAvcInitAppInfo(RIVP_APP_LDD_CMN_PARAM_T **app, RIVP_LDD_BOOL existResource)
{
	...
	if (app != RIVP_LDD_NULL) {
		...
		if (lBoolRet == RIVP_LDD_TRUE) {
			...
			lAppPtr->useRingBuff		= RIVP_LDD_TRUE;	/* use ring buffer, or not */ /* [T.B.D.] TRUE case is not tested in this application */
			lAppPtr->picIndex			= 0u;
			lAppPtr->strmInfo.strmId	= 0u;
			lAppPtr->errorCode			= RIVP_LDD_RTN_OK;
			lAppPtr->displayMode		= RIVP_APP_DISPLAY;
			lAppPtr->yuvBuff			= s_rivpAppLddAvcYuvBuff;
			lAppPtr->yuvBuffCnt			= 0u;
			lAppPtr->streamFrameId		= 0u;
