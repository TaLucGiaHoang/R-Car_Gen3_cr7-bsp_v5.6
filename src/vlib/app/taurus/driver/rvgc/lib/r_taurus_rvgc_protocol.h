...
#define RVGC_PROTOCOL_IOC_SURFACE_ALPHA_SET                  ((TAURUS_PROTOCOL_RVGC_ID << 24) | 0xF00008)

struct taurus_rvgc_ioc_surf_alpha_set_in {
    uint64_t cookie;
    uint32_t display;
    uint32_t layer;
    uint8_t alpha;
...
struct taurus_rvgc_ioc_surf_alpha_set_out {
    uint64_t cookie;
    uint64_t res;
} __packed;
...
struct taurus_rvgc_cmd_msg {
    ...
    union {
        ...
        struct taurus_rvgc_ioc_surf_alpha_set_in ioc_surf_alpha_set;
    } params;
};
...
struct taurus_rvgc_res_msg {
    ...
    union {
        ...
        struct taurus_rvgc_ioc_surf_alpha_set_out ioc_surf_alpha_set;
