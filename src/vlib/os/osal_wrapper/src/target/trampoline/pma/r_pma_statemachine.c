...
#include "target/trampoline/pma/r_pma_common.h"
#include "target/trampoline/pma/r_pma_statemachine.h"
    ...
    while (true)
    {
        ...
        if (LenOsalRet == OSAL_RETURN_OK)
        {
            ...
            if (LenOsalRet == OSAL_RETURN_OK)
            {
                ...
                {
                    LenResult = R_OSAL_Sys_KickTaskById(LstMsgToTask.xUserTaskHandle);
                    if (LenResult) R_OSAL_ThreadSleepForTimePeriod(10);
                    break;
                }
            }
            else
...
e_result_t R_PMA_SM_SetClockStat(st_cpg_domain_t * pCpgDomain, bool bStat)
{
    ...
    if (true == bStat)
    {
        ...
        {
            pCpgDomain->refCnt++;
            /* TODO: Workaround for DHD safety group */
...
e_result_t R_PMA_SM_SetPowerStat(st_sysc_domain_t * pSyscDomain, bool bStat)
{
    ...
    TickType startTick, endTick, tickDiff;
    uint32_t         pwrStat;
