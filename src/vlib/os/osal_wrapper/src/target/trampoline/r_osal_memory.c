...
* Product Name : OSAL
* Device(s) :    R-Car
* Description :  OSAL wrapper Memory Manager for Trampoline
***********************************************************************************************************************/
...
* @retval           OSAL_RETURN_FAIL
***********************************************************************************************************************/
OSAL_STATIC e_osal_return_t osal_internal_mem_send_monitor_event (base_mutex_t * p_mutex_id,
...
e_osal_return_t osal_memory_initialize(void)
{
    ...
    else
    {
        ...
        else
        {
            ...
            ret = base_mutex_create(&gs_osal_mem_mmngr_ptr_self->inner_mtx);
...
e_osal_return_t osal_memory_deinitialize(void)
{
    ...
    if (NULL != gs_osal_mem_mmngr_ptr_self)
    {
        base_mutex_destroy(&gs_osal_mem_mmngr_ptr_self->inner_mtx);
...
OSAL_STATIC e_osal_return_t osal_internal_mem_open(const st_osal_mmngr_config_t * const p_usr_config)
{
    ...
    ret = base_mutex_lock_timeout(&p_mngr_control->inner_mtx, p_mngr_control->inner_timeout);
    if (OSAL_RETURN_OK != ret)
    {
        osal_ret = OSAL_RETURN_FAIL;
    }
    else
    {
        ...
        base_mutex_unlock(&p_mngr_control->inner_mtx);
    }
                                                          ...
                                                          const st_osal_mmngr_config_t * const p_usr_config)
{
    ...
    if (OSAL_RETURN_OK == osal_ret)
    {
        ...
        {
            ret = base_mutex_create(&p_mngr_control->p_buffers[loop_count].inner_mtx);
            if (OSAL_RETURN_OK != ret)
            {
                osal_ret = OSAL_RETURN_MEM;
            }
        ...
        if (OSAL_RETURN_OK != osal_ret)
        {
            ...
            {
                (void)base_mutex_destroy(&p_mngr_control->p_buffers[destroy_count].inner_mtx);
                destroy_count++;
            }
            vPortFree(p_mngr_control->p_monitor);
    ...
    else
    {
        ...
        if (OSAL_RETURN_OK == osal_ret)
        {
            ...
            if (OSAL_RETURN_OK == osal_ret)
            {
                ...
                {
                    base_mutex_destroy(&p_mngr_control->p_buffers[loop_count].inner_mtx);
                                                           ...
                                                           osal_memory_buffer_handle_t buffer_obj)
{
    ...
    /* osal_mutex must be already protected */
    ret = base_mutex_lock_timeout(&p_buffer_info->inner_mtx, p_buffer_info->inner_timeout);
    if (OSAL_RETURN_OK != ret)
    {
        osal_ret = OSAL_RETURN_FAIL;
    }
    else
    {
        ...
        base_mutex_unlock(&p_buffer_info->inner_mtx);
    }
                                                                  ...
                                                                  osal_memory_buffer_handle_t buffer_hndl)
{
    ...
    /* osal_mutex must already be protect */
    base_mutex_unlock(p_mutex_id);
    ...
    if (OSAL_RETURN_OK == osal_ret)
    {
        ret = base_mutex_lock_timeout(p_mutex_id, OSAL_MEMORY_INNER_TIMEOUT);
        if (OSAL_RETURN_OK != ret)
        {
            osal_ret = OSAL_RETURN_FAIL;
        }
...
OSAL_STATIC e_osal_return_t osal_internal_mem_getcpuptr(void* p_ptr_self, void** const pp_cpu_ptr)
{
    ...
    else
    {
        ...
        else
        {
            ...
            {
                GetResource(osal_mutex);
                ret = base_mutex_lock_timeout(&p_buffer->inner_mtx, p_buffer->inner_timeout);
                if (OSAL_RETURN_OK != ret)
                {
                    osal_ret = OSAL_RETURN_FAIL;
                }
                else
                {
                    *pp_cpu_ptr = p_buffer->p_buffer_va;
                    ...
                    base_mutex_unlock(&p_buffer->inner_mtx);
                }
                ReleaseResource(osal_mutex);
            }
        }
