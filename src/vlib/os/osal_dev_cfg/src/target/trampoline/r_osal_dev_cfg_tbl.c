...
st_osal_device_info_t* osal_device_info(void)
{
    ...
    Function body
    ------------------------------------------------------------------------------------------------------------------*/
    const uint32_t soc = R_OSAL_DCFG_GetSocType();

    switch (soc) {
        case (uint32_t)OSAL_DEVICE_SOC_TYPE_E3:
            return gs_osal_device_info_e3;
        case (uint32_t)OSAL_DEVICE_SOC_TYPE_H3:
            return gs_osal_device_info_h3;
        case (uint32_t)OSAL_DEVICE_SOC_TYPE_M3:
            return gs_osal_device_info_m3;
        case (uint32_t)OSAL_DEVICE_SOC_TYPE_M3N:
